function Invoke-Test
{
    ...
    $parametersConPtyShell = @($Rows, $Cols, $CommandLine)
    Add-Type -TypeDefinition $Source -Language CSharp;
    $output = [DummyShellUpgrade]::ConPtyShellMain($parametersConPtyShell)
    Write-Output $output
}
...
public static class ConPtyShell
{
    ...
    public static string SpawnConPtyShell(uint rows, uint cols, string commandLine, bool upgradeShell)
    {
...
public static class DummyShellUpgrade
{
    ...
    private static uint ParseRows(string[] arguments)
    {
        ...
        if (arguments.Length > 1)
            rows = (uint)CheckInt(arguments[1]);
        return rows;
    }
    ...
    private static uint ParseCols(string[] arguments)
    {
        ...
        if (arguments.Length > 0)
            cols = (uint)CheckInt(arguments[0]);
        return cols;
    }
    ...
    public static string ConPtyShellMain(string[] args)
    {
        ...
        try
        {
            ...
            string commandLine = "powershell.exe";
            output = ConPtyShell.SpawnConPtyShell(rows, cols, commandLine, upgradeShell);
        }
        catch (Exception e)
        {
            Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(DummyShellUpgrade.ConPtyShellMain(args));
    }
}
